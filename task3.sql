...
---------------1
select client.id, client.secondName, client.firstName
from cheque
inner join client on client.id = cheque.id
--------
select client.id, client.secondName, client.firstName
from cheque
left outer join client on client.id = cheque.id
--------
select client.id, client.secondName, client.firstName
from cheque
right outer join client on client.id = cheque.id
--------
select client.id, client.secondName, client.firstName
from cheque
full outer join client on client.id = cheque.id
--------
select positions.salary, employees.firstName, employees.secondName, employees.shopID
from positions cross join employees
--------
select equipment.Name, allservices.serviceName
from allservices cross apply equipment
--------
select client.id table1, client.firstName, client.secondName, client.age
...
select *
from ((select client.firstName, client.secondName from client where client.gender = 'бигендер' )
union (select client.firstName, client.secondName from client where client.age > 32)) table1;
--------
select *
from ((select automobile.yearOfManufacture, automobile.mileage from automobile)
union all (select automobile.markID, automobile.modelID from automobile where automobile.markID = 3)) table1;
--------
select * 
from ((select employees.firstName, employees.secondName from employees where employees.shopID = 1)
except (select employees.firstName, employees.secondName from employees where employees.employmentDate = '20180209')) table1;
--------
select * 
from ((select employees.firstName, employees.secondName from employees where employees.shopID = 1)
intersect (select employees.firstName, employees.secondName from employees where employees.employmentDate = '20180209')) table1;

------------------3
select client.firstName, client.secondName, client.gender
from client
where exists (select client.id from client where age > 30 and gender = 'мужчина')
--------
select automobile.markID, automobile.mileage
from automobile
where automobile.yearOfManufacture in (2010, 2020)
--------
select *
from automobile
where id = some (select id from automobile where mileage > 145000)
--------
select *
from allservices
where price >= all (select price from allservices where id > 1)
--------
select *
from allservices
where allservices.price between '60' and '200'
--------
select *
from client
where patronymic like 'Сергеев%'

------------------4
select 
case 
when yearOfManufacture > 2018 then 'Автомобиль нового поколения' else 'Автомобиль старого поколения'
end as SortedByYearOfManufacture
from automobile

-------------------5
select 'Время открытия магазина ' + cast(openingTime as char(5))
from branchOffice

select 'Время открытия магазина ' + convert(char(5), openingTime)
from branchOffice
--------
select isnull(adress, 'адрес отсутствует(') table1
from branchOffice
--------
select nullif(20, number)
from spareParts
--------

select coalesce(iif(shopID = 0, 'Работник не имеет офиса',
choose(shopID, 'Офис 1', 'Офис 2', 'Офис 3')), 'нет') as BranchOfficesAndEmployees
from employees

------------------6
select replace(id, 1, 'Администрация')
from branchOffice
--------
select substring(adress, 1, 15)
from branchOffice
--------
select stuff(firstName, len(firstName)- 2, 5, 'ыч')
from employees
--------
select str(avg(cast(id as float)), 1) as id
from automobile
--------
select lower(unicode(name))
from positions
--------
...
------------------7
select datepart(year, employmentDate) yearOfEmployment, datepart(month, employmentDate) monthOfEmployment
from employees
--------
select dateadd(year, 12, employmentDate) 
from employees
--------
select datediff(month, (select min(employmentDate)
 from employees
 ), employmentDate) 
from employees
--------
...
------------------8
select automobile.yearOfManufacture, max(yearOfManufacture)
from automobile
group by yearOfManufacture
having yearOfManufacture > 2010